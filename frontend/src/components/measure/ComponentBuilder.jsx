...
const COMPONENT_TYPES = [
  { value: 'diagnosis', label: 'Diagnosis', icon: Stethoscope, description: 'Medical condition or disease (ICD-10, SNOMED)' },
  { value: 'encounter', label: 'Encounter', icon: Calendar, description: 'Patient visit or healthcare interaction (CPT, HCPCS)' },
  { value: 'procedure', label: 'Procedure', icon: ClipboardCheck, description: 'Medical procedure performed (CPT, HCPCS, ICD-10-PCS)' },
  { value: 'observation', label: 'Observation/Lab', icon: FileText, description: 'Lab result, vital sign, or clinical observation (LOINC)' },
  { value: 'medication', label: 'Medication', icon: Pill, description: 'Drug or medication (RxNorm, NDC)' },
  { value: 'demographic', label: 'Demographic', icon: Users, description: 'Patient demographics (age, sex, etc.)' },
  { value: 'assessment', label: 'Assessment', icon: ClipboardCheck, description: 'Clinical assessment or screening result' },
];
...
export function ComponentBuilder({
  ...
  // Component state
  const [componentType, setComponentType] = useState('diagnosis');
  const [description, setDescription] = useState('');
  const [logicOperator, setLogicOperator] = useState('AND');

  // Value set state
  const [valueSetMode, setValueSetMode] = useState('existing');
  ...
  const [newValueSetCodeSystem, setNewValueSetCodeSystem] = useState('ICD10');

  // Timing state
  const [timingPreset, setTimingPreset] = useState('measurement_period');
  const [customTimingValue, setCustomTimingValue] = useState(12);
  const [customTimingUnit, setCustomTimingUnit] = useState('months');
  const [customTimingDirection, setCustomTimingDirection] = useState('before');
  ...
  const handleSave = () => {
    ...
    if (valueSetMode === 'existing' && selectedValueSetId) {
      ...
      newValueSet = {
        ...
        confidence: newValueSetOid ? 'high' : 'medium',
        source: 'User Created',
        verified: false,
        codes: [],
        totalCodeCount: 0,
    ...
    const component = {
      ...
      confidence: valueSetRef?.oid ? 'high' : 'medium',
      source: 'User Created',
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <select
                    ...
                    onChange={(e) => setNewValueSetCodeSystem(e.target.value)}
            ...
            {timingPreset === 'custom' && (
              <div className="flex items-center gap-2 p-3 bg-[var(--bg-tertiary)] rounded-lg border border-[var(--border)]">
                ...
                <select
                  ...
                  onChange={(e) => setCustomTimingUnit(e.target.value)}
                ...
                <select
                  ...
                  onChange={(e) => setCustomTimingDirection(e.target.value)}
                  className="px-2 py-1 bg-[var(--bg-secondary)] border border-[var(--border)] rounded text-[var(--text)]"
                >
