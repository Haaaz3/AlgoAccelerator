...
function getComplexityDots(level) {
  switch (level) {
    case 'low':
      return '●○○';
    case 'medium':
...
function AtomicDetails({ component }) {
  ...
  const allValueSets = component.valueSets || (component.valueSet ? [component.valueSet] : []);
...
function ComplexitySection({ component }) {
...
export function ComponentDetail({ componentId, onClose, onEdit }) {
  ...
  const canArchive =
    (component.usage?.usageCount || 0) === 0 && status !== 'archived';
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)] overflow-hidden">
          ...
          >
            <span className="flex items-center gap-2">
              ...
              Version History
              <span className="text-xs text-[var(--text-dim)]">
                ({component.versionInfo?.versionHistory?.length || 0})
              </span>
            </span>
            {versionHistoryOpen ? (
        ...
        <div className="rounded-lg border border-[var(--border)] bg-[var(--bg)] px-4 py-3">
          ...
          <p className="text-sm text-[var(--text-muted)]">
            ...
            <span className="font-semibold text-[var(--text)]">
              {component.usage?.usageCount || 0}
            </span>{' '}
            {(component.usage?.usageCount || 0) === 1 ? 'measure' : 'measures'}
          </p>
