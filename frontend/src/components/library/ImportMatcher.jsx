...
function MatchedComponentRow({ match, decision, onDecisionChange }) {
...
export default function ImportMatcher({ onComplete, onCancel }) {
  ...
  // Handlers
  // --------------------------------------------------------------------------
  const setMatchDecision = useCallback((name, decision) => {
    setMatchDecisions((prev) => ({ ...prev, [name]: decision }));
  }, []);

  const toggleAddToLibrary = useCallback((name) => {
  ...
  const setNewCategory = useCallback((name, category) => {
    setNewDecisions((prev) => ({
      ...prev,
      [name]: {
        ...prev[name],
