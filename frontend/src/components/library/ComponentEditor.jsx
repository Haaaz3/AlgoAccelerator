...
import SharedEditWarning from './SharedEditWarning.jsx';
...
export default function ComponentEditor({ componentId, onSave, onClose }) {
  ...
  const [timingOperator, setTimingOperator] = useState(
    existingComponent?.type === 'atomic' ? existingComponent.timing?.operator ?? 'during' : 'during'
  );
  ...
  const [timingReference, setTimingReference] = useState(
    existingComponent?.type === 'atomic' ? existingComponent.timing?.reference ?? 'Measurement Period' : 'Measurement Period'
  );
  const [negation, setNegation] = useState(
    existingComponent?.type === 'atomic' ? existingComponent.negation ?? false : false
  );
  ...
  const [operator, setOperator] = useState(
    existingComponent?.type === 'composite' ? existingComponent.operator ?? 'AND' : 'AND'
  );
  ...
  const createAtomicComponent = (params) => {
    ...
    return {
      ...
      complexity: {
        score: 1 + (params.timing.quantity ? 1 : 0) + (params.timing.position ? 1 : 0) + (params.negation ? 1 : 0),
        level: 'low',
        factors: { base: 1, timingClauses: params.timing.quantity ? 1 : 0, negations: params.negation ? 1 : 0 },
      },
      metadata: {
  ...
  const createCompositeComponent = (params) => {
    const id = `comp-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    const childrenCount = params.children.length;
    return {
      id,
      type: 'composite',
      name: params.name,
      ...
      complexity: {
        score: childrenCount + (params.operator === 'AND' ? 1 : 0),
        level: childrenCount <= 2 ? 'low' : childrenCount <= 4 ? 'medium' : 'high',
        factors: { base: 1, childrenSum: childrenCount, andOperators: params.operator === 'AND' ? 1 : 0 },
      },
      metadata: {
        category: params.category,
        categoryAutoAssigned: false,
  ...
  const handleSave = useCallback(() => {
    ...
    if (componentType === 'atomic') {
      ...
      const component = createAtomicComponent({
        ...
        timing: {
          ...
          position: timingPosition || undefined,
    ...
    codes, categoryAutoAssigned, syncComponentToMeasures, measures, batchUpdateMeasures,
  ]);
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      ...
      {showSharedWarning && existingComponent && (
        <SharedEditWarning
          ...
          usageCount={existingComponent.usage?.usageCount ?? 0}
            ...
            if (pendingChanges) {
              ...
              const duplicated = {
                ...
                versionInfo: {
                  ...
                  versionId: (parseFloat(existingComponent.versionInfo?.versionId ?? '1.0') + 0.1).toFixed(1),
                  status: 'draft',
                },
              };
