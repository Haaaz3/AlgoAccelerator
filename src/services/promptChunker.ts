...
import type { ExtractedMeasureData } from './aiExtractor';
...
export interface ChunkingOptions {
  /** Maximum characters per chunk (default: 100,000 for most LLMs) */
  maxChunkSize: number;
  /** Characters of overlap between chunks (default: 2,000) */
  overlapSize: number;
  /** Whether to preserve section headers in each chunk */
  preserveHeaders: boolean;
  /** Minimum chunk size before combining with next (default: 5,000) */
  minChunkSize: number;
}
...
export interface DocumentChunk {
  ...
  endOffset: number;
  /** Detected section types in this chunk */
...
export interface DocumentStructure {
  /** Detected measure ID */
  measureId?: string;
  /** Detected measure title */
  title?: string;
  /** All detected sections */
  sections: ChunkSection[];
  /** Estimated complexity based on section count */
  complexity: 'simple' | 'moderate' | 'complex';
}

export interface MergeResult {
  /** Merged extraction result */
  merged: ExtractedMeasureData;
  /** Chunks that contributed to each field */
  contributions: MergeContributions;
  /** Any conflicts detected during merge */
  conflicts: MergeConflict[];
}

export interface MergeContributions {
  populations: Record<string, number[]>; // population type -> chunk indices
  valueSets: Record<string, number[]>; // value set OID -> chunk indices
}

export interface MergeConflict {
  field: string;
  chunkValues: Array<{ chunkIndex: number; value: unknown }>;
  resolution: 'first' | 'longest' | 'merged';
}
 ...
 */
export function detectSections(content: string): ChunkSection[] {
 ...
 */
function getSectionsBefore(sections: ChunkSection[], offset: number): ChunkSection[] {
...
export function mergeChunkResults(
  results: Array<{ chunk: DocumentChunk; extraction: ExtractedMeasureData }>
): MergeResult {
  if (results.length === 0) {
  ...
  const merged: ExtractedMeasureData = {
    ...base,
    populations: [],
    valueSets: [],
  };
  ...
  const populationsByType = new Map<string, Array<{ pop: any; chunkIndex: number }>>();
  ...
  for (const [type, instances] of populationsByType) {
    ...
    if (instances.length === 1) {
      ...
      const criteriaDescriptions = new Set<string>();
  ...
  const valueSetsByOid = new Map<string, Array<{ vs: any; chunkIndex: number }>>();
  const valueSetsByName = new Map<string, Array<{ vs: any; chunkIndex: number }>>();
