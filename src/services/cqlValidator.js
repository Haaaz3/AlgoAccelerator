 ...
 */
export function validateCQLSyntax(cql        )                      {
  const errors                       = [];
  const warnings                         = [];
 ...
 */
function checkBalancedDelimiters(cql        )                       {
  const errors                       = [];
 ...
 * Check for common syntax issues
 */
function checkCommonSyntaxIssues(cql        , lines          )                       {
  const errors                       = [];
 ...
 */
function extractCQLMetadata(cql        )                                  {
...
function parseTranslatorErrors(data         )                       {
  ...
  if (typeof data === 'object' && data !== null) {
    ...
    if (obj.errorExceptions) {
      return obj.errorExceptions.map((e     ) => ({
        severity: 'error'         ,
        code: 'TRANSLATION_ERROR'                ,
...
function parseELMAnnotations(
  ...
  return annotations
    .filter((a     ) => a.errorSeverity === severity)
    .map((a     ) => ({
      severity,
      code: severity === 'error' ? 'TRANSLATION_ERROR'                 : 'DEPRECATED_SYNTAX'                  ,
      message: a.message,
