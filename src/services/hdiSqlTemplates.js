...
export function generateOntologyCTE(config                     )         {
...
export function generateDemographicsCTE(config                     )         {
...
export function generateEncounterPredicateCTE(
  ...
  if ((predicate       ).codes) {
    ...
    if (codes.valueSetOid) {
      ...
      const codeList = codes.explicitCodes.map((c     ) => `'${c.code}'`).join(', ');
