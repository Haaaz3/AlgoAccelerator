...
function detectLookbackPeriod(description        , valueSetName         )                                           {
...
function generateCQLForDataElement(
  element             ,
  measurementPeriodRef         = '"Measurement Period"'
)         {
...
export function generateClauseCode(
  ...
  const componentSQLs = clause.children.map(child => {
    if ('operator' in child && 'children' in child) {
      return `(\n${generateClauseCode(child                 , format, overrides, populationId)}\n)`;
    } else {
      const element = child               ;
 ...
 */
export function generateCQLDefinitionName(description        )         {
  return description
 ...
 */
export function generateSQLAlias(description        , index        )         {
 ...
 */
export function generateAtomicComponentCode(component                 )                             {
  const warnings           = [];
  const errors           = [];
 ...
 */
function generateDemographicComponentCode(component                 )                             {
  // Handle gender check
 ...
 */
function generateComponentSQLTiming(timing                              , dataModel        )                {
 ...
 */
function getDataModelFromResource(resourceType        )         {
  const models                         = {
    Encounter: 'ENCOUNTER',
    Condition: 'CONDITION',
    Procedure: 'PROCEDURE',
 ...
 */
function getSQLTableName(dataModel        )         {
  const tables                         = {
 ...
 */
function getSQLDateColumn(dataModel        )         {
  const columns                         = {
 ...
 */
function getSQLStatusCheck(dataModel        )                {
  const checks                         = {
 ...
 */
function convertToSQLInterval(quantity        , unit        )                                  {
  const unitMap                         = {
    years: 'YEAR',
    year: 'YEAR',
    months: 'MONTH',
 ...
 */
function getComponentResourceType(component                 )                {
 ...
 */
export function createGeneratedCode(result                            )                         {
  return {
    cql: result.cql,
    sql: result.sql,
