...
export const DEFAULT_HDI_CONFIG                      = {
...
function extractPredicatesFromUMS(
  ...
  const generateAlias = (prefix        ) => `PRED_${prefix}_${++predicateCounter}`;
...
function extractFromLogicalClause(
  ...
  for (const child of clause.children) {
    if ('operator' in child && 'children' in child && !('type' in child)) {
      ...
      const nested = extractFromLogicalClause(child                 , generateAlias, valueSets);
 ...
 */
function extractTimingFromElement(element             )   
...
function generatePredicateCTE(
  ...
  switch (predicate.type) {
    case 'demographics':
      return generateDemographicsPredicateCTE(predicate                         , config);
    case 'condition':
      return generateConditionPredicateCTE(predicate                      , config);
    case 'result':
      return generateResultPredicateCTE(predicate                   , config);
    case 'procedure':
      return generateProcedurePredicateCTE(predicate                      , config);
    case 'medication':
      return generateMedicationPredicateCTE(predicate                       , config);
    case 'immunization':
      return generateImmunizationPredicateCTE(predicate                         , config);
    case 'encounter':
      return generateEncounterPredicateCTE(predicate                      , config);
    default: {
 ...
 */
function generateFinalCalculation(mapping                   )         {
 ...
 */
function estimateComplexity(mapping                   )                            {
