 ...
 */
export async function parseMeasureSpec(file      )                           {
  const warnings           = [];
  const parsingNotes           = [];
  ...
  try {
    ...
    if (['html', 'htm'].includes(fileExtension)) {
      ...
      parsingNotes.push(`Parsed eCQM HTML: found ${Object.keys(parsedData).filter(k => parsedData[k                           ]).length} fields`);
 ...
 */
async function parseECQMHtml(file      )                             {
 ...
 */
async function parseExcelSpec(file      )                             {
 ...
 */
async function parsePdfSpec(file      )                             {
  const data                    = {
    measureId: null,
    title: null,
    description: null,
 ...
 */
async function parseStructuredSpec(file      , format        )                             {
...
async function parseZipPackage(file      )                           {
  const warnings           = [];
  const parsingNotes           = ['Processing ZIP package...'];
 ...
 */
function extractClinicalCodes(text        )               {
  const codes               = [];
  const found = new Set        ();
 ...
 */
function extractValueSets(text        )                   {
  const valueSets                   = [];
...
function mapCodeSystem(systemStr        )             {
...
function mapFhirPopulationType(code        )         {
  const map                         = {
 ...
 */
function generateUMSFromParsedData(data                   , filename        )                       {
 ...
 */
function toFHIRPopulationType(type        )                 {
  const mapping                                 = {
...
function convertParsedPopulations(data                   )                         {
      ...
      if (encounterMatch) {
        ...
        children.push({
          ...
          valueSet: encVs ? {
            ...
            codes: encVs.codes,
            confidence: encVs.oid ? 'high'                    : 'medium'                   ,
 ...
 */
function findValueSetByKeywords(vsMap                             , keywords          )                             {
 ...
 */
function determineOperator(narrative        , criteria          )                       {
...
function parseNarrativeForCriteria(
  ...
  const clinicalTerms = [
    { pattern: /(?:hospice|palliative)\s+care/gi, type: 'procedure'          },
    { pattern: /(?:immunodeficiency|HIV|AIDS)/gi, type: 'diagnosis'          },
    { pattern: /(?:cancer|leukemia|lymphoma|myeloma)/gi, type: 'diagnosis'          },
    { pattern: /(?:screening|exam|test|assessment)\s+(?:performed|completed)/gi, type: 'procedure'          },
  ];
...
function convertParsedValueSets(data                   )                      {
  const measureId = data.measureId || 'parsed';
...
function generateDefaultPopulations(data                   )                         {
...
function extractMeasureIdFromFilename(filename        )         {
...
function generateFallbackUMS(filename        )                       {
  const now = new Date().toISOString();
