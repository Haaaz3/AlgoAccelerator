 ...
 */
function validatePopulation(pop                      , path        , issues                   )       {
  if (!pop.id) {
 ...
 */
function validateClause(clause               , path        , issues                   )       {
  if (!clause.id) {
  ...
  if (!clause.children || clause.children.length === 0) {
    ...
    clause.children.forEach((child, i) => {
      if ('operator' in child) {
        validateClause(child                 , `${path}.children[${i}]`, issues);
      } else {
        validateDataElement(child               , `${path}.children[${i}]`, issues);
      }
    });
  }
}
 ...
 */
function validateDataElement(element             , path        , issues                   )       {
  if (!element.id) {
  ...
  if ((element       ).temporalConstraints) {
    issues.push({
      severity: 'info',
 ...
 */
function validateAgeConsistency(measure                      )                    {
  const issues                    = [];
  const foundAges                                                      = [];
  ...
  const findAges = (node     , path        ) => {
    ...
    if (node.children) {
      node.children.forEach((child     , i        ) => findAges(child, `${path}.children[${i}]`));
    }
  };
