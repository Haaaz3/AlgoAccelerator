...
export function syncAgeConstraints(
  ...
  const updated = JSON.parse(JSON.stringify(measure))                        ;
...
function syncClauseAgeConstraints(
  ...
  for (const child of clause.children) {
    if ('operator' in child) {
      ...
      syncClauseAgeConstraints(child                 , newMin, newMax);
    } else {
      // It's a DataElement
      const element = child               ;
 ...
 */
function isAgeRelatedElement(element             )          {
 ...
 * - "21 through 64"
 */
function updateAgeInText(text        , newMin        , newMax        )         {
  if (!text) return text;
 ...
 */
export function extractAgeRange(measure                      )                                      {
 ...
 */
function findAgeElement(clause               )                     {
  for (const child of clause.children) {
    if ('operator' in child) {
      const found = findAgeElement(child                 );
 ...
 */
export function detectAgeInconsistencies(measure                      )           {
  const issues           = [];
  const ageRanges                                                      = [];
...
function checkClauseForAgeRanges(
  ...
  for (const child of clause.children) {
    if ('operator' in child) {
      checkClauseForAgeRanges(child                 , ageRanges, populationType);
    } else {
      const element = child               ;
