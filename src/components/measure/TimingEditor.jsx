...
function BoundaryEditor({ boundary, onChange, label: _label }                     ) {
  ...
  const handleOffsetValueChange = (value        ) => {
    onChange({ ...boundary, offsetValue: value });
  };

  const handleOffsetUnitChange = (unit            ) => {
    onChange({ ...boundary, offsetUnit: unit });
  };

  const handleOffsetDirectionChange = (direction                    ) => {
      ...
      {hasOffset ? (
        <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-[var(--bg-secondary)] rounded border border-[var(--border)]">
          ...
          <select
            ...
            onChange={(e) => handleOffsetUnitChange(e.target.value              )}
