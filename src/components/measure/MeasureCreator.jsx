...
import { CriteriaBlockBuilder } from './CriteriaBlockBuilder';
...
export function MeasureCreator({ isOpen, onClose }                     ) {
  ...
  const [mode, setMode] = useState              ('ai_guided');
  ...
  const [denominatorCriteria, setDenominatorCriteria] = useState                    ({
    description: '',
    valueSets: new Set(),
    criteriaBlocks: [],
  });
  ...
  const convertToCriteriaBlocks = useCallback((criteria                        )                  => {
    return criteria.map((crit, idx) => {
      const block                = {
        ...
        type: 'criterion',
        resourceType: crit.resourceType                                 ,
      ...
      if (crit.timing && crit.timing.type) {
        block.timing = {
          type: crit.timing.type                                                                                                                                                ,
          value: crit.timing.value,
          unit: crit.timing.unit                                           ,
          ageValue: crit.timing.ageValue,
          ageUnit: crit.timing.ageUnit                                           ,
      ...
      if (crit.quantity && crit.quantity.comparator) {
        block.quantity = {
          comparator: crit.quantity.comparator                                             ,
  ...
  const handleCreate = async () => {
    ...
    if (mode === 'copy' && sourceMeasureId) {
      ...
      const allSelectedVSIds = new Set        ([
  ...
  const convertCriteriaBlockToUMS = (block               )                              => {
    if (block.type === 'group') {
      return {
        id: block.id,
  ...
  const buildPopulationsFromCriteria = ()                         => {
    const populations                         = [];
...
function resetClauseReviewStatus(clause               )                {
  return {
    ...clause,
    reviewStatus: 'pending',
    children: clause.children.map(child => {
      if ('operator' in child) {
        return resetClauseReviewStatus(child                 );
      }
      return { ...child, reviewStatus: 'pending' };
    }),
  };
}
